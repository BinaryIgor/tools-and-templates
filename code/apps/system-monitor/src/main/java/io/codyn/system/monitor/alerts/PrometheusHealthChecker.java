...
public class PrometheusHealthChecker {
    ...
    public PrometheusHealthChecker(MeterRegistry meterRegistry,
                                   ...
                                   @Value("${prometheus-health-check.retry-times}") int retryTimes,
                                   @Value("${prometheus-health-check.retry-interval}") int retryInterval) {
    ...
    public void check() {
        try {
            ...
            if (checkPrometheusHealth(request)) {
                ...
                prometheusDownTimestampGauge.set(Metrics.secondsTimestamp(System.currentTimeMillis()));
